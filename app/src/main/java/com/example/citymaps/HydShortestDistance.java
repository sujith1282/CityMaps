...
public class HydShortestDistance extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hyd_shortest_distance);
    ...
    public class setDType{
        int dist;
        String place;
        setDType(int d,String p){
            this.dist=d;
            this.place=p;
        }
    }
    ...
    public void dijkstraD(String src,String dest,HashMap<String, List<pair>> adj,HashMap<String,String> parent,HashMap<String,Integer> distances){
        PriorityQueue<setDType> pq = new PriorityQueue<>(Comparator.comparingInt(s -> s.dist));
        distances.put(src,0);
        pq.add(new setDType(0,src));
        while(!pq.isEmpty()){
            setDType top=pq.poll();
            String p=top.place;
            int d=top.dist;
            ...
            for(pair nbr : adj.get(p)){
                if(d+nbr.dist<distances.get(nbr.place)){
                    ...
                    pq.add(new setDType(d+nbr.dist,nbr.place));
                }
            }
